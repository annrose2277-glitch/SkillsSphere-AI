...
const InterviewResults = () => {
    ...
    return (
      <div className="results-container">
        <div className="session-error-state">
          ...
          <button className="btn-back" onClick={() => navigate("/mock-interview")}>
            Back to Lobby
          </button>
        </div>
      </div>
  ...
  const avgTechnical = answers.length
    ? Math.round(answers.reduce((s, a) => s + (a.scores?.technical || 0), 0) / answers.length)
    : 0;
  const avgCommunication = answers.length
    ? Math.round(answers.reduce((s, a) => s + (a.scores?.communication || 0), 0) / answers.length)
    : 0;
  const avgRelevance = answers.length
    ? Math.round(answers.reduce((s, a) => s + (a.scores?.relevance || 0), 0) / answers.length)
    : 0;
    ...
    <div className="results-container">
      ...
      <div className="results-header">
        ...
        <div className="results-meta">
          <span className="results-badge badge-topic">{results.topic?.toUpperCase()}</span>
          <span className="results-badge badge-difficulty">{results.difficulty}</span>
          {results.duration && (
      ...
      <div className="overall-score-card">
        <div className="overall-ring">
          <svg width="140" height="140">
            <circle cx="70" cy="70" r="56" fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth="10" />
            <circle
      ...
      {results.weakConcepts?.length > 0 && (
        <div className="weak-concepts-card">
          <h3>
            <AlertTriangle size={16} style={{ color: "#f59e0b", marginRight: 8, display: "inline" }} />
            Concepts to Review
          </h3>
          <div className="concept-badges">
            {results.weakConcepts.map((c, i) => (
              <span key={i} className="concept-badge missed">{c}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {answers.map((a, idx) => (
        <div key={idx} className="answer-card">
          <div className="answer-card-header" onClick={() => toggleCard(idx)}>
            ...
            <span className="answer-card-score" style={{ color: getScoreColor(a.scores?.technical || 0) }}>
              {a.scores?.technical || 0}%
            </span>
            {expandedCards[idx] ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
          </div>
          {expandedCards[idx] && (
            <div className="answer-card-body">
              <p><strong>Your Answer:</strong> {a.transcript || "No answer submitted"}</p>
              <div className="answer-scores-row">
                <span className="answer-score-pill">Tech: {a.scores?.technical || 0}%</span>
                <span className="answer-score-pill">Comm: {a.scores?.communication || 0}%</span>
                <span className="answer-score-pill">Rel: {a.scores?.relevance || 0}%</span>
              </div>
              {a.concepts && (
                <div className="concept-badges" style={{ marginTop: "0.5rem" }}>
                  {a.concepts.detected?.map((c, i) => (
                    <span key={`d-${i}`} className="concept-badge detected">✓ {c}</span>
                  ))}
                  {a.concepts.missed?.map((c, i) => (
                    <span key={`m-${i}`} className="concept-badge missed">✗ {c}</span>
                  ))}
                </div>
              )}
            </div>
      ...
      <div className="results-actions">
        <button className="btn-retake" onClick={() => navigate("/mock-interview")}>
          <RefreshCw size={16} /> New Interview
        </button>
        <button className="btn-history" onClick={() => navigate("/mock-interview/history")}>
